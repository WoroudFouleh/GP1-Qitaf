...
exports.getOverallStatistics = async (req, res) => {
    try {
        const usersCount = await User.countDocuments({ userType: 2 }); // Count users with type = 2
        const landownersCount = await User.countDocuments({ userType: 1 }); // Count users with type = 1

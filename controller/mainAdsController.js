const MainAd = require('../model/mainAdvertisements');

exports.getAllAdvertisements = async (req, res) => {
    try {
        const ads = await MainAd.find().sort({ publishingDate: -1 }); // Sort by most recent
...
exports.deleteAdvertisement = async (req, res) => {
    try {
        ...
        const deletedAd = await MainAd.findByIdAndDelete(id);
        if (!deletedAd) {
...
exports.editAdvertisement = async (req, res) => {
    try {
        ...
        const updatedAd = await MainAd.findByIdAndUpdate(
            id,
            { image },
